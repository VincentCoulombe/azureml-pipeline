...
class ScriptWrapper():
    ...
    def get_csv_from_input_folder(self, csv_name:str) -> pd.DataFrame:
        if "input_folder" not in self.args_list: raise ValueError("""Aucun input folder de reçu en argument. Ce script est probablement la première étape du pipeline.
                                                       Utilisez get_csv_from_argument() si vous souhaitez charger un Dataset qui ne provient pas d'une étape précédente du pipeline.""")
    ...
    def save_csv_in_output_folder(self, dataframe:pd.DataFrame, saving_name:str, index:bool=False, header:bool=True):
        if "output_folder" not in self.args_list: raise ValueError("""Aucun output folder de reçu en argument. Ce script est probablement la dernière étape du pipeline.""")
        if not saving_name.endswith(".csv"): saving_name += ".csv"
    ...
    def get_csv_from_argument(self, csv_name:str, argument_name:str) -> pd.DataFrame:      
        if argument_name not in self.args_list: raise ValueError(f"{argument_name} n'est pas dans la liste d'arguments reçus. Soit : {self.args_list}.")
        if not csv_name.endswith(".csv"): csv_name += ".csv"
